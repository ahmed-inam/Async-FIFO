// Read-side driver + monitor + sequence. Owns the sequence-to-driver mailbox;
// the monitor-to-scoreboard mailbox comes from the env.
class fifo_rd_agent #(parameter int DATA_WIDTH = 32);

  virtual fifo_rd_if #(DATA_WIDTH)          vif;
  mailbox #(fifo_transaction #(DATA_WIDTH)) mon_mbx;

  fifo_rd_driver   #(DATA_WIDTH) drv;
  fifo_rd_monitor  #(DATA_WIDTH) mon;
  fifo_rd_sequence #(DATA_WIDTH) seq;
